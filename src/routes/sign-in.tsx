...
export const Route = createFileRoute("/sign-in")({
  ...
  head: () => ({ meta: [{ title: "Sign In — DevPath" }] }),
});
...
function SignInPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate({ to: u.role === "admin" ? "/admin" : "/profile" });
    } catch (e: any) {
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 grid place-items-center px-4 py-10">
        <div className="w-full max-w-md">
          ...
          <div className="rounded-2xl border border-border bg-card p-6 sm:p-8 mt-0 rounded-t-none border-t-0">
            <div className="text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Welcome back to DevPath.</p>
            </div>
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative mt-1.5">
                  <input type={show ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full rounded-md border border-border bg-input px-3 py-2 pr-9 text-sm outline-none focus:border-primary" />
              ...
              <p className="text-center text-xs text-muted-foreground">Don't have an account? <Link to="/sign-up" className="text-primary font-semibold">Create Account</Link></p>
